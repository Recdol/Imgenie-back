...
class PlaylistIdExtractor:
    ...
    def _set_up_path(self, config: AppConfig):
        ...
        self.SIT_SUB = (
            f"sit/{config.sit_model_version}/{config.sit_model_version}_huggingface"
        )
        self.MOOD_SUB = (
            f"mood/{config.mood_model_version}/{config.mood_model_version}_huggingface"
        )
        ...
        self.WEATHER_FAISS_PATH = os.path.join(
            self.FAISS_PATH, f"weather/{config.weather_index_version}.index"
        )
        self.SIT_FAISS_PATH = os.path.join(
            self.FAISS_PATH, f"sit/{config.sit_index_version}.index"
        )
        self.MOOD_FAISS_PATH = os.path.join(
    ...
    def _load_dataset(self):
        ...
        self.weather_dataset.load_faiss_index(
            index_name="embeddings", file=self.WEATHER_FAISS_PATH
        )

        self.sit_dataset = self._read_dataset(self.SIT_DATA_PATH)
        self.sit_dataset.load_faiss_index(
            index_name="embeddings", file=self.SIT_FAISS_PATH
        )

        self.mood_dataset = self._read_dataset(self.MOOD_DATA_PATH)
        self.mood_dataset.load_faiss_index(
            index_name="embeddings", file=self.MOOD_FAISS_PATH
        )

    def _load_model(self, config: AppConfig):
        self.weather_processor = AutoImageProcessor.from_pretrained(
            config.model_repo, subfolder=self.WEATHER_SUB
        )
        self.weather_model = AutoModel.from_pretrained(
            config.model_repo, subfolder=self.WEATHER_SUB
        )

        self.sit_processor = AutoImageProcessor.from_pretrained(
            config.model_repo, subfolder=self.SIT_SUB
        )
        self.sit_model = AutoModel.from_pretrained(
            config.model_repo, subfolder=self.SIT_SUB
        )

        self.mood_processor = AutoImageProcessor.from_pretrained(
            config.model_repo, subfolder=self.MOOD_SUB
        )
        self.mood_model = AutoModel.from_pretrained(
            config.model_repo, subfolder=self.MOOD_SUB
        )
    ...
    def get_weather_playlist_id(self, image_path: str) -> tuple[list[float], list]:
        ...
        scores, retrieved_examples = self._get_similar_images_topk(
            ...
            k=self.config.playlist_k,
        )
        return scores, [
            retrieved_examples["playlist_id"][i] for i in range(self.config.playlist_k)
        ]
    ...
    def _get_similar_images_topk(
        ...
        scores, retrieved_examples = dataset.get_nearest_examples(
            "embeddings", query_embedding.squeeze(), k=k
        )
